...
const App = () => {
  ...
  const [selectedBook, setSelectedBook] = useState(
    localStorage.getItem("lastBook") || "gn",
  );
  const [selectedChapter, setSelectedChapter] = useState(
    Number(localStorage.getItem("lastCap")) || 1,
  );
  ...
  const apiToken =
    ...
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdHIiOiJNb24gTWFyIDMwIDIwMjYgMTc6NTk6NTMgR01UKzAwMDAuZnNjbGF1ZGlvQGdtYWlsLmNvbSIsImlhdCI6MTc3NDg5MzU5M30.7dLxSUyQcv_xMthRt8DYm9II8kZZCL3mZqdt9ZK5QU8";
      ...
      for (const p of paths) {
        try {
          ...
          if (!ct.includes("application/json")) {
            ...
            console.warn(
              `Tentativa ${p} retornou tipo ${ct}; início do corpo:`,
              txt.slice(0, 300),
            );
            continue;
          }
      ...
      for (const p of paths) {
        try {
          ...
          if (!ct.includes("application/json")) {
            const txt = await res.text();
            console.warn(
              `POST tentativa ${p} retornou tipo ${ct}; início do corpo:`,
              txt.slice(0, 300),
            );
            continue;
          }
  ...
  const doSearch = useCallback(async () => {
    if (!searchQuery || searchQuery.trim().length === 0)
      return setSearchError("Digite um termo para buscar");
      ...
      try {
        ...
        try {
          const cached = JSON.parse(
            localStorage.getItem(BOOKS_CACHE_KEY) || "null",
          );
          ...
          console.warn(
            "Cache de livros inválido, continuando para fetch remoto",
          );
        }
        ...
        for (const p of tryPaths) {
          try {
            ...
            if (r.status === 429) {
              ...
              setLoadError(
                "Limite de requisições atingido para /books. Aguarde ou autentique-se para remover limites.",
              );
              break;
            }
            if (!r.ok) {
            ...
            if (!ct.includes("application/json")) {
              ...
              console.warn(
                `Tentativa ${p.url} retornou tipo ${ct}; corpo inicial:`,
                txt.slice(0, 300),
              );
              continue;
            }
            ...
            if (
              ...
              console.warn(
                "normalizeBooks retornou vazio; usando payload bruto como fallback para popular books",
              );
              normalized = json;
            }
            if (mounted) {
            ...
            try {
              ...
              console.warn("Falha ao salvar cache de livros:", err);
            }
            success = true;
            break;
          } catch (e) {
            console.warn(
              `Erro ao tentar ${p.url}:`,
        ...
        if (!success) {
          console.error(
            "Não foi possível obter lista de livros em nenhuma rota tentada. Veja os logs acima para detalhes.",
          );
          ...
          try {
            ...
            if (fb.ok) {
              ...
              if (mounted) {
                ...
                setBooksLoaded(true);
                setLoadError(
                  "Usando lista de livros local (fallback) - API remota indisponível.",
                );
              }
            } else {
              if (mounted)
                setLoadError(
                  "Não foi possível carregar a lista de livros (incluindo fallback). Veja o console para detalhes.",
                );
            }
          } catch (e) {
            ...
            if (mounted)
              setLoadError(
                "Não foi possível carregar a lista de livros. Veja o console para detalhes.",
              );
          }
        }
      } catch (err) {
  ...
  const exportFavorites = () => {
    if (favorites.length === 0)
      return alert("Sua lista de favoritos está vazia!");
  ...
  const toggleFavorite = (verse) => {
    ...
    let newFavs = isFav
      ? favorites.filter((f) => f.text !== verse.text)
      : [
          ...favorites,
          { ...verse, bookName: selectedBook, cap: selectedChapter },
        ];
      ...
      <header style={styles.header}>
        <header
          style={{
            ...
            borderRadius: isMobile ? "0" : "12px", // Bordas retas no mobile, arredondadas no desk
            marginBottom: "30px",
            textAlign: "center",
        ...
        >
          ...
          >
            Bíblia Completa Em Versões
          </h1>
          <p style={{ color: "#e0e0e0", marginTop: "10px", fontSize: "1rem" }}>
            Compare traduções simultaneamente e organize seus estudos.
          </p>
        </header>
        <div style={styles.controls}>
          ...
          <div style={styles.selectContainer}>
            ...
            >
              {books.map((b) => (
                <option key={b.abbrev.pt} value={b.abbrev.pt}>
                  {b.name}
                </option>
              ))}
            </select>
          ...
          <button onClick={() => setSelectedChapter((prev) => prev + 1)}>
            ➡️
          </button>
          <div
            style={{
          ...
          >
            ...
            {!apiToken && (
              <span
                style={{ color: "#a33", fontSize: "0.8rem", marginLeft: 8 }}
              >
                Sem token: limite 20 req/h
              </span>
            )}
          </div>
        </div>
      ...
      {searchResults && searchResults.length > 0 && (
        ...
        >
          ...
          {searchResults.map((r, i) => (
            ...
            >
              <strong>
                {r.book_name || r.book || ""} {r.chapter ?? r.cap}:
                {r.number ?? r.verse}
              </strong>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
            {favorites.map((f, i) => (
              <div key={i} style={styles.favItem}>
                <strong>
                  {(f.bookName ?? "").toString().toUpperCase()} {f.cap}:
                  {f.number}
                </strong>
                <p style={{ fontSize: "0.85rem" }}>
                  {(f.text ?? "").toString().substring(0, 30)}...
                </p>
                <button
            ...
            <div key={v} style={styles.column(index)}>
              <h2
                style={{
                  ...
                  paddingBottom: "5px",
                  fontSize: "1.1rem",
                  backgroundColor: "#007bff", // Azul (você pode trocar por #28a745 para verde ou #6f42c1 para roxo)
                  color: "#fff",
                }}
              >
                {apiVersions.find((api) => api.version === v)?.verses ||
                  v.toUpperCase()}
              </h2>
              {content[v]?.verses?.map((verse, idx) => (
                <div
                  key={`${v}-${verse?.number ?? idx}`}
                ...
                >
                  <button
                    ...
                    style={{
                      ...styles.btnStar,
                      color: favorites.some((f) => f.text === verse.text)
                        ? "#f1c40f"
                        : "#ccc",
                    }}
                  >
                  ...
                  <p>
                    ...
                    {(() => {
                      ...
                      const baseVerse = content[baseline]?.verses?.find(
                        (bv) => bv.number === verse.number,
                      ) || { text: "" };
                      return (
                        <DiffVerse
                          leftText={baseVerse.text}
                          rightText={verse.text}
                        />
                      );
                    })()}
                  </p>
...
const styles = {
  ...
  selectField: {
    ...
    backgroundImage:
      "url(\"data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://w3.org' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'%3E%3Cpolyline points='6 9 12 15 18 9'%3E%3C/polyline%3E%3C/svg%3E\")",
      ...
      return (
        <div style={{ padding: 20 }}>
          ...
          <div style={{ marginTop: 12 }}>
            <button onClick={this.handleReload} style={{ padding: "8px 12px" }}>
              Recarregar (limpar estado)
            </button>
          </div>
        </div>
      );
